      ...
      <div :style="{ padding: '24px', background: '#fff', minHeight: '360px' }">
        ...
        <a-collapse :bordered="false" id="resume-collapse">
          <a-collapse-panel header="个人信息" key="1">
            <a-icon
              type="edit"
              :style="{ float: 'right', color: '#1890FF', fontSize: '20px', cursor: 'pointer'}"
              title="编辑"
              @click="showModal(personalInformation, '个人信息')"
            />
...
export default {
  ...
  methods: {
    ...
    informationHandler(personalInformation, title) {
      console.log(personalInformation, title);
    },
    handleOk(){
      this.visible = false
    },
    showModal(personalInformation, title) {
      this.visible = true
      this.title = 
    },
  }
